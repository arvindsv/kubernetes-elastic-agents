...
public class PluginRequestTest {
    @Test
    public void shouldSendAConsoleLogAppenderCall() {
        ...
        when(accessor.submit(any())).thenReturn(DefaultGoApiResponse.success("All good"));

        final PluginRequest pluginRequest = new PluginRequest(accessor);
        pluginRequest.appendToConsoleLog(jobIdentifier, "text1");

        verify(accessor).submit(requestCaptor.capture());

        Map params = new Gson().fromJson(requestCaptor.getValue().requestBody(), Map.class);
        assertThat(params.get("pipelineName"), is("p1"));
        assertThat(params.get("pipelineCounter"), is("1"));
        assertThat(params.get("stageName"), is("s1"));
        assertThat(params.get("stageCounter"), is("1"));
        assertThat(params.get("jobName"), is("j1"));
        assertThat(params.get("text"), is("text1"));
    }
